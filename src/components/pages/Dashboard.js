...
export const ETongue = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                setETongueData(updatedData);

                // ✅ CALL ML API AFTER DATA FETCH
                callMLModel(updatedData);
    ...
    const callMLModel = async (data) => {
        try {
            const payload = {
                ...
                g: Number(data.g?.slice(-1)[0]) || 0,
                b: Number(data.b?.slice(-1)[0]) || 0,
                freq: Number(data.fq?.slice(-1)[0]) || 0
            };

            console.log("FINAL PAYLOAD:", payload);

            const res = await fetch(`${BeURL}/predict-taste`, {
                method: "POST",
                headers: {
            ...
            setMlPrediction(result.taste);  
            setType(result.type);    

            // Fake confidence (you can improve later)
            setConfidence(90);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50">
            ...
            <DashboardHeader isConnected={true} onDownload={handleDownload} />
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-8 py-8 lg:py-10">
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <ChemicalSensorCard label="pH Level" value={eTongueData.ph} unit="pH" />
                        <ChemicalSensorCard label="TDS" value={eTongueData.dts} unit="ppm" />
                        <ChemicalSensorCard label="VOC" value={eTongueData.voc} unit="ppm" />
                    </div>
                </div>
